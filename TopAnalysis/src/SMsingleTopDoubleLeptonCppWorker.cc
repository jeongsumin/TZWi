#include "../interface/SMsingleTopDoubleLeptonCppWorker.h"
#include <iostream>
#include <cmath>

using namespace std;

SMsingleTopDoubleLeptonCppWorker::SMsingleTopDoubleLeptonCppWorker(const std::string modeName)
{
...
typedef SMsingleTopDoubleLeptonCppWorker::TRAF TRAF;
typedef SMsingleTopDoubleLeptonCppWorker::TRAI TRAI;
typedef SMsingleTopDoubleLeptonCppWorker::TRAB TRAB;

void SMsingleTopDoubleLeptonCppWorker::setElectrons(TRAF pt, TRAF eta, TRAF phi, TRAF mass, TRAI charge,
...
void SMsingleTopDoubleLeptonCppWorker::setMuons(TRAF pt, TRAF eta, TRAF phi, TRAF mass, TRAI charge,
...
void SMsingleTopDoubleLeptonCppWorker::setJets(TRAF pt, TRAF eta, TRAF phi, TRAF mass,
                                         TRAI id, TRAF CSVv2) {
  in_Jet_p4[0] = pt;
  in_Jet_p4[1] = eta;
  in_Jet_p4[2] = phi;
...
void SMsingleTopDoubleLeptonCppWorker::setMET(TTreeReaderValue<float>* pt, TTreeReaderValue<float>* phi) {
  in_MET_pt = pt;
  in_MET_phi = phi;
}

void SMsingleTopDoubleLeptonCppWorker::resetValues() {
...
//signal muons
bool SMsingleTopDoubleLeptonCppWorker::isGoodMuon(const unsigned i) const {
  const double pt = in_Muons_p4[0]->At(i);
  const double eta = in_Muons_p4[1]->At(i);
  if ( pt < minSubMuonPt_ or std::abs(eta) > maxMuonEta_ ) return false;
...
//Loose(veto) muons
bool SMsingleTopDoubleLeptonCppWorker::isLooseMuon(const unsigned i) const{
  const double pt = in_Muons_p4[0]->At(i);
  const double eta = in_Muons_p4[1]->At(i);
  if( pt < minSubMuonPt_ or std::abs(eta) > maxMuonEta_ ) return false;
  if ( ! (in_Muons_isPFcand->At(i) != 0 and (in_Muons_isGlobal->At(i) != 0 or in_Muons_isTracker->At(i) != 0)) ) return false;
  if ( in_Muons_relIso->At(i) > maxLooseMuonRelIso_ ) return false; //maxLooseMuonRelIso : Loose PF isolation value

  return true;
}
//signal electrons
bool SMsingleTopDoubleLeptonCppWorker::isGoodElectron(const unsigned i) const {
  const double pt = in_Electrons_p4[0]->At(i) * in_Electrons_eCorr->At(i);
  const double eta = in_Electrons_p4[1]->At(i);
  if ( pt < minSubElectronPt_ or std::abs(eta) > maxElectronEta_ ) return false;
...
//veto electrons
bool SMsingleTopDoubleLeptonCppWorker::isVetoElectron(const unsigned i) const {
  const double pt = in_Electrons_p4[0]->At(i) * in_Electrons_eCorr->At(i);
  const double eta = in_Electrons_p4[1]->At(i);
  if ( pt < minSubElectronPt_ or std::abs(eta) > maxElectronEta_ ) return false;
  //nanoAOD object -> Electron_cutBased_Sum16 0:fail, 1:veto, 2:medium, 3:tight
...
bool SMsingleTopDoubleLeptonCppWorker::isGoodJet(const unsigned i) const {
...
TLorentzVector SMsingleTopDoubleLeptonCppWorker::buildP4(const TRAF p4Arr[], unsigned i) const {
  TLorentzVector p4;
  p4.SetPtEtaPhiM(p4Arr[0]->At(i), p4Arr[1]->At(i), p4Arr[2]->At(i), p4Arr[3]->At(i));
  return p4;
}

bool SMsingleTopDoubleLeptonCppWorker::analyze() {
  resetValues();
  ...
  */
  ...
  // Build Z candidate with leptons
  ...
  // Sort jets by pt and CSVv2iminator
